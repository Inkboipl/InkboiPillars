...
public class PillarsDiM2Teleporter {
	public static Holder<PoiType> poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegisterEvent event) {
		event.register(Registries.POINT_OF_INTEREST_TYPE, registerHelper -> {
			PoiType poiType = new PoiType(ImmutableSet.copyOf(PillarsdimeModBlocks.PILLARS_DI_M_2_PORTAL.get().getStateDefinition().getPossibleStates()), 0, 1);
			registerHelper.register(ResourceLocation.parse("pillarsdime:pillars_di_m_2_portal"), poiType);
	...
	public PillarsDiM2Teleporter(ServerLevel level) {
		this.level = level;
	}
	...
	public Optional<BlockUtil.FoundRectangle> createPortal(BlockPos p_77667_, Direction.Axis p_77668_) {
		...
		BlockState blockstate = PillarsdimeModBlocks.PILLARS_DI_M_2_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
